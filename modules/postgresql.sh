...
module_run() {
  ...
  if [[ -n "${FORGE_ENVIRONMENT:-}" ]]; then
    create_database_if_configured "$FORGE_ENVIRONMENT"
  fi
...
create_database_if_configured() {
  ...
  local db_name_var="POSTGRESQL_DB_${env_upper}"
  local db_user_var="POSTGRESQL_USER_${env_upper}"
  local db_pass_var="POSTGRESQL_PASSWORD_${env_upper}"
